...
void PriortyTwice:: Priorty_Twice::set_new_process(string name, int run, int arrive, int prio_num)
{
	count++;
	index_num++;
	processes.add_process_parameter(name, run, arrive, prio_num, index_num);
}

void PriortyTwice:: Priorty_Twice::calc_waiting_time()
{
...
float PriortyTwice:: Priorty_Twice::average_fun()
{
	int sum = 0;
	process* current = processes.head->next;
	while (current != NULL)
	...
	total_average = (float)sum / count;
 return total_average;
}

void PriortyTwice:: Priorty_Twice::sort()
{
...
void PriortyTwice:: Priorty_Twice::non_premptive()
{
	...
	if (processes.head != NULL && processes.head->next != NULL)
	{
		...
		{
			if (((processes.head->priority) - (current->arrived_time)) >= (current->priority))
			{
					...
					if (temporary->next != NULL)
					{
						while (temporary->next != NULL)
						{
							if ((temporary->priority) > (temporary->next->priority))
							{
								...
								temporary->priority = temporary->next->priority;
								...
								temporary->next->priority = temp_data.priority;
					...
					if (temporary->next != NULL)
					{
						while (temporary->next != NULL)
						{
							if ((temporary->running_time) > (temporary->next->running_time))
							{
								...
								////////////////////////////////////////////
								temporary->next->process_name = temp_data.process_name;
								temporary->index = temp_data.index;
								temporary->next->priority = temp_data.priority;
								temporary->next->running_time = temp_data.running_time;
								temporary->next->arrived_time = temp_data.arrived_time;
								///////////////////////////////////////////
		...
		else
		{
			...
			if (((current_moving->priority)) >= (current->priority))
			{
...
void PriortyTwice:: Priorty_Twice::premptive()
{
	process* current = processes.head;
	while (current->next != NULL)
	{
...

	{
		...
		if (current->arrived_time <= (processes.head->running_time))
		{
			...
			if (((processes.head->priority) - (current->arrived_time)) >= (current->priority))
			{
				...
				/////////////////////////////
				processes_temp.add_process_parameter(processes.head->process_name, first_running_time, processes.head->arrived_time, processes.head->priority, processes.head->index);
				current_temp = processes_temp.head;
				processes_temp.add_process_parameter(current->process_name, current->running_time, current->arrived_time, current->priority, current->index);
				current_temp = current_temp->next;
				////////////////////////////////////////////////
				remain_time = ((processes.head->running_time) - (current->arrived_time));
				if (remain_time <= 0) { remain_time = 0; }
				///////////////////////////////////////////////
                processes_temp.add_process_parameter(processes.head->process_name, remain_time, current->arrived_time, processes.head->priority, processes.head->index);
				current_temp = current_temp->next;
		...
		else
		{
			...
			if (((current_moving->running_time) /*- (current->arrived_time)*/) >= (current->running_time))
			{
				...
				if (current->arrived_time > current_moving->running_time)
					...
					first_running_time = current->arrived_time;
				/////////////////////////////
				processes_temp.add_process_parameter(current_moving->process_name, first_running_time, current_moving->arrived_time, current_moving->priority, current_moving->index);
				current_temp = processes_temp.head;
				processes_temp.add_process_parameter(current->process_name, current->running_time, current->arrived_time, current->priority, current->index);
				current_temp = current_temp->next;
				////////////////////////////////////////////////
				...
				if (remain_time <= 0) { remain_time = 0; }
				///////////////////////////////////////////////
                processes_temp.add_process_parameter(current_moving->process_name, remain_time, current->arrived_time, current_moving->priority, current_moving->index);
				current_temp = current_temp->next;
...
void PriortyTwice:: Priorty_Twice::is_burst_equal_zero()
{
	process* current = processes.head;
...
PriortyTwice:: process* PriortyTwice:: Priorty_Twice::return_value_to_mostafa()
{
	return processes.head;
}

void PriortyTwice:: Priorty_Twice::non_premptive_method()
{
	non_premptive();
	calc_waiting_time();
	average_fun();
...
void PriortyTwice:: Priorty_Twice::premptive_method()
{
	premptive();
	calc_waiting_time();
	average_fun();
...
PriortyTwice:: process_queue::process_queue()
{
	head = NULL;
}

void PriortyTwice:: process_queue::add_process_parameter(string data, int running, int arriving, int priority_num, int index_number)
{
	process* temp;
	temp = new process;
	temp->process_name = data;
	temp->running_time = running;
	temp->priority = priority_num;
